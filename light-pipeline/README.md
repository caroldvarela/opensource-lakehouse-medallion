# Data Stack Framework - Lightweight Lakehouse Architecture with Medallion

This project implements a lightweight Lakehouse architecture using the Medallion structure (Bronze → Silver → Gold) with open source tools for data ingestion, processing, orchestration, and querying of Stack Overflow data.
...
### Medallion Structure (Lightweight Version)

```
Bronze Layer (Raw Data)
├── Raw data in Parquet format
├── Overwrite allowed
├── Source: StackOverflow Dataset (votes 2021 + manual tables)
└── Simplified annual data

Silver Layer (Curated Data)
├── Iceberg tables with MERGE writes
├── Curated, normalized and historical data
├── Includes load_date column
└── Deduplication and data cleaning

Gold Layer (Analytics Ready)
├── Iceberg tables with MERGE writes
├── Simplified KPIs, metrics and aggregations
├── Includes load_date column
...
### 4. Execute Lightweight ETL Pipeline
...
#### Bronze Layer (Ingestion with DLT + Manual)
- **Source**: StackOverflow Dataset from public S3
- **Tables**: 
  - **DLT**: `votes 2021` (157 MB) - Automatic ingestion
  - **Manual**: `votes 2020` (176 MB), `posts 2020` (2.9 GB), `posts 2021` (2.4 GB) - Manual upload required
- **Format**: Parquet
...
│   ├── bronze.py                   # Bronze ingestion with DLT (votes 2021 only)
